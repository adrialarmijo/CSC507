"""
Read First Half Module
======================

    Read the first half of each file specified in the file_paths and calculate the sum.

    Args:
        file_paths (list): A list of file paths.

    Returns:
        int: The sum of the values in the first half of the files.
...
if __name__ == "__main__":
    ...
    for file in file_paths:
        with open(file) as f:
            lines = f.readlines()
        half = len(lines) // 2
        first_half_sum = sum_calculator.calculate(lines[:half]) # first half
        print(first_half_sum) # prints to stdout
